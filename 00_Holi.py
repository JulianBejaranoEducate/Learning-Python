#Hola mundo print 
#Imprime "¡Hola Mundo!" por consola
print ("Holi perro")
print ("Mi nombre: Sara y tengo 20 años")
"""
Un tipo de dato es una forma de clasificar 
diferentes valores con diferentes "categorias"
"""
''''
Este es otro comentario
de  varias líneas
'''
#Consultar el tipo de dato: 
print (type("Holi perro")) #Tipo "str"
print (type(5)) #Tipo "int"
print (type(1.5)) #Tipo "float"
print (type(True)) #Tipo "boolean"

#Pedir nombre al usuario
print ("Por favor, ingrese su nombre")
nombre= input()
print ("¡Hola " + nombre+"!")
print (2+5)
#Imprime un tipo de dato de una operación númrica
print (type (2+5))

print ( type(print("Typo de dato print SORPRESA no tiene tipo de dato,es una funcion del sistema")))
